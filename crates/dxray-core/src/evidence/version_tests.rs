//! What [`stamp_versions`](super::stamp_versions) reports, each way it can
//! have no number, and that a directory is read once however often it is asked.
...
fn a_shipped_dlss_runtime_reports_which_build_it_is() {
    // The name is always `nvngx_dlss.dll`; only the version tells builds apart.
...
fn a_version_resource_of_all_zeroes_is_a_version_and_not_a_missing_one() {
    // An all-zero resource stays a number, not "no version".
...
fn a_local_file_with_no_version_is_not_the_same_as_a_name_windows_resolves() {
    // No resource is a fact about the file; no local copy is a fact about the
    // directory. Both are printed.
...
fn a_file_that_is_not_a_pe_image_is_unreadable_rather_than_unversioned() {
    // A zero-byte placeholder is reported as such.
...
fn a_proxy_dll_shadowing_a_system_name_is_versioned_because_it_is_what_loads() {
    // A local `dxgi.dll` copy is what the loader takes, so its version is read.
...
fn a_library_is_matched_against_the_directory_spelling_not_the_import_table() {
    // The file is found whatever case the import spelled it in.
...
fn one_file_named_by_two_findings_carries_the_same_version_under_both() {
    // A library behind two signals is read once per verdict.
...
fn a_verdict_nobody_stamped_says_so_rather_than_claiming_the_file_is_elsewhere() {
    // `analyse` opens nothing, so its signals are `Unread`, not "elsewhere".
...
fn a_signal_reads_as_one_sentence_with_the_version_inside_the_provenance() {
    // One rendering, shared by both binaries.
...
/// Removes a file, to prove a cache never goes back to disk: access times are
/// unreliable under `relatime` and `noatime`.
...
fn a_library_already_read_for_one_executable_is_not_read_again_for_the_next() {
    // One read per file for the whole walk: an 803-file folder used to open each
    // DLSS runtime 805 times.
...
fn one_read_answers_both_the_link_chase_and_the_version() {
    // Following the link leaves the version already known.
...
fn the_directory_is_listed_once_however_many_files_in_it_are_inspected() {
    // The directory is listed once, not once per record.
...
fn a_cache_that_moves_to_another_directory_forgets_the_first_one() {
    // The cache holds one directory, so names never cross between games.
